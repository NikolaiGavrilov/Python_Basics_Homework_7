...
def rhythm_check(string_to_check):
    phrases_to_check = string_to_check.split()
    vowels_list = 'аеёиоуыэюя'
    
    count_vowels = lambda x: sum(1 for x in x if x in vowels_list)

    temp = count_vowels(phrases_to_check[0])
    for i in range(len(phrases_to_check)):
        if temp != count_vowels(phrases_to_check[i]):
            return 'Пам парам'
    return 'Парам пам-пам'

user_string = 'пара-ра-рам рам-пам-папам па-ра-па-да'
print(rhythm_check(user_string))